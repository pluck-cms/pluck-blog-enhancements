...
function blog_page_admin_editpost() {
	global $lang, $var1, $cont1, $cont2, $cont3, $cont4, $cont5, $cont6;
	...
	if (isset($_POST['save']) || isset($_POST['save_exit'])) {
		if (seo_url($cont1)) {
			//Save blogpost.
			if($cont4 && $cont5)
				$force_time = array(explode('-', $cont4), explode(':', $cont5));
			else
				$force_time = null;

			$seoname = blog_save_post($cont1, $cont2, $cont3, $var1, $force_time, $cont6);
	...
	?>
		...
		<form method="post" action="">
			...
			<p>
				<label class="kop2" for="cont4">Time</label>
				Date: <input name="cont4" id="cont4" type="text" value="<?php echo blog_post_date($var1); ?>" /> (dd-mm-yyyy)
				<br />
				Time: <input name="cont5" id="cont5" type="text" value="<?php echo blog_post_time($var1); ?>" /> (hh:mm:ss)
			</p>
			<p>
				<label class="kop2" for="cont3"><?php echo $lang['general']['contents']; ?></label>
				<textarea class="<?php if (defined('WYSIWYG_TEXTAREA_CLASS')) echo WYSIWYG_TEXTAREA_CLASS; ?>" name="cont3" id="cont3" cols="70" rows="20"><?php echo htmlspecialchars_decode($post['content']); ?></textarea>
			</p>
			<p>
				<label class="kop2" for="cont6">settings</label> 
				<input type="checkbox" name="cont6" <?php if ($post['allow_reaction']) echo 'checked="checked"'; ?> /> Allow reactions for this post
			</p>
...
function blog_page_admin_newpost() {
	global $lang, $var1, $cont1, $cont2, $cont3, $cont4;

	//If form is posted...
	if (isset($_POST['save']) || isset($_POST['save_exit'])) {
		if (seo_url($cont1)) {
			//Save blogpost.
			$seoname = blog_save_post($cont1, $cont2, $cont3, null, null, $cont4);
	...
	?>
		...
		<form method="post" action="">
			...
			<p>
				<label class="kop2" for="cont4">settings</label> 
				<input type="checkbox" name="cont4" checked="checked " /> Allow reactions
			</p>
			<?php show_common_submits('?module=blog', true); ?>
		</form>
	<?php
