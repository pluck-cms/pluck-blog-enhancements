...
function blog_admin_module_settings_beforepost() {
	...
	echo '<span class="kop2">'.$lang['blog']['title'].'</span>
		<table>
			...
			<tr>
				<td><input type="checkbox" name="reverse_posts" id="reverse_posts" value="true" '; if (module_get_setting('blog','reverse_posts') == 'true') { echo 'checked="checked" '; } echo '/></td>
				<td><label for="reverse_posts">&emsp;Newest post at the top of a page</label></td>
			</tr>
			<tr>
...
function blog_admin_module_settings_afterpost() {
	...
	if (empty($_POST['posts_per_page']))
		...
		$settings = array(
			...
			'reverse_posts' => (isset($_POST['reverse_posts'])) ? 'true' : 'false',
...
$blog_cat_prefix = '&amp;module=blog&amp;page=viewcat&amp;cat=';
run_hook('blog_cat_prefix', array(&$blog_cat_prefix));
define('BLOG_CAT_PREFIX', $blog_cat_prefix);
unset($blog_cat_prefix);
?>