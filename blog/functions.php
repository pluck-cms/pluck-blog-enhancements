 ...
 */
function blog_save_post($title, $category, $content, $current_seoname = null, $force_time = null, $allow_reaction = 1) {
	...
	$content = sanitizePageContent(trim($content));
	...
	if (is_null($force_time))
		$post_time = time();
	elseif (is_array($force_time)) {
		if (count($force_time, COUNT_RECURSIVE) == 8) {
			$post_time = mktime($force_time[1][0], $force_time[1][1], $force_time[1][2], $force_time[0][0], $force_time[0][1], $force_time[0][2]);
		}
		else
			$post_time = time();
	}
	...
	$data['allow_reaction'] = $allow_reaction;
...
function blog_get_posts() {
	...
	if ($files) {
		...
		if (module_get_setting('blog','reverse_posts') == 'true')
			$files = array_reverse($files);
...
function blog_post_date_year($post = null) {
	if(is_null($post))
		return date('Y');
	else
		//pobrac dane z postu i wstrawiæ
		echo 'dsa';
}
function blog_post_date($post = null) {
	if (!empty($post) && (blog_get_post_filename($post))) {
		include BLOG_POSTS_DIR.'/'.blog_get_post_filename($post);
		return date('d-m-Y', $post_time);
	}
	else
		return date('d-m-Y', time());
}
function blog_post_time($post = null) {
	if (!empty($post) && (blog_get_post_filename($post))) {
		include BLOG_POSTS_DIR.'/'.blog_get_post_filename($post);
		return date('H:i:s', $post_time);
	}
	else
		return date('d-m-Y', time());
}

//Get title of category from seoname
function blog_get_category_seoname($title) {
global $lang;

	$cats = read_dir_contents(BLOG_CATEGORIES_DIR, 'files');
	if ($cats) {
		foreach($cats as $cat) {
			include BLOG_CATEGORIES_DIR.'/'.$cat;
			if ($title == $category_title)
				return str_replace('.php', '', $cat);
		}
		unset($cat);
	}
	//If we are here...
	return seo_url($lang['blog']['no_cat']);
}

//List of posts in category
function blog_post_in_category($seoname) {
if ($title = blog_get_category_title($seoname)) {
	$files = read_dir_contents(BLOG_POSTS_DIR, 'files');
	if ($files) {
		natcasesort($files);
		foreach ($files as $file) {
			include BLOG_POSTS_DIR.'/'.$file;
			if ($post_category == $seoname) {
				$posts[] = array(
					'title'   => $post_title,
					'seoname' => blog_get_post_seoname($file)
				);
			}
		}
		unset($file);
