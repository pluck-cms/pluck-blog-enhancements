...
function blog_pages_site() {
	...
	//if we are on category page
	if (isset($_GET['cat']) && blog_category_exists($_GET['cat'])) {
		$module_page_admin[] = array(
				'func'  => 'viewcat',
				'title' => blog_get_category_title($_GET['cat'])
			);
	}

	//if we are on category 'no category' page
	elseif (isset($_GET['cat']) && $_GET['cat'] == seo_url($lang['blog']['no_cat'])) {
		$module_page_admin[] = array(
				'func'  => 'viewcat',
				'title' => $lang['blog']['no_cat']
			);
	}

	//If category doesn't exist, make page title 404 error
	else {
		$module_page_admin[] = array(
				'func'  => 'viewcat',
				'title' => $lang['general']['404']
			...
			if ($category == null || $category == $post['category']) {
				...
				<div class="blog_post">
					...
					<span class="blog_post_info">
						<?php echo $post['date'].' '.$lang['blog']['at'].' '.$post['time'].' '.$lang['blog']['in'].' <a href="'.PAGE_URL_PREFIX.CURRENT_PAGE_SEONAME.BLOG_CAT_PREFIX.blog_get_category_seoname($post['category']).'" title="'.$post['category'].'">'.$post['category'].'</a>'; ?>
						<br/>
					...
					if (module_get_setting('blog','allow_reactions') == 'true' && $post['allow_reaction']) {
...
// View category page
function blog_page_site_viewcat() {
	//Global language variables.
	global $lang;
	$posts = blog_post_in_category($_GET['cat']);

	if ($posts) {
		echo '<ul id="posts_list">';
		foreach ($posts as $post) {
			echo '<li><a href="'.PAGE_URL_PREFIX.CURRENT_PAGE_SEONAME.BLOG_URL_PREFIX.$post['seoname'].'" title="'.$post['title'].'">'.$post['title'].'</a></li>';
		}
		unset($post);
		echo '</ul>';
	}
